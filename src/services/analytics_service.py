...
def get_daily_money(db: Session, target_date: datetime) -> Dict[str, Any]:  # Отчёт о доходах и расходах за день
    if not isinstance(target_date, datetime):  # Проверяем тип даты
        raise TypeError("Дата должна быть datetime объектом")  # Бросаем ошибку типа
    if target_date > datetime.now():  # Проверяем, что дата не в будущем
        raise ValueError("Дата отчёта не может быть в будущем")  # Бросаем ошибку значения

    ticket_income, snack_income = 150000, 50000  # Временные данные о доходах
    staff_costs, rent_costs, product_costs = 80000, 40000, 60000  # Временные данные о расходах

    total_income = ticket_income + snack_income  # Суммарный доход
    total_costs = staff_costs + rent_costs + product_costs  # Суммарные расходы
    profit = total_income - total_costs  # Прибыль как разница доходов и расходов

    return {  # Возвращаем агрегированный отчёт
        'date': target_date.date(),
        'income': {'tickets': ticket_income, 'snacks': snack_income, 'total': total_income},
        'costs': {'staff': staff_costs, 'rent': rent_costs, 'products': product_costs, 'total': total_costs},
        'profit': profit,
        'profit_margin': round(profit / total_income * 100, 2) if total_income > 0 else 0
    }


def get_period_money(db: Session, start_date: datetime, end_date: datetime) -> Dict[str, Any]:  # Отчёт о доходах и расходах за период
    if not isinstance(start_date, datetime) or not isinstance(end_date, datetime):  # Проверяем типы дат
        raise TypeError("Даты начала и окончания должны быть datetime объектами")  # Бросаем ошибку типа
    if start_date > end_date:  # Проверяем корректность диапазона
        raise ValueError("Дата начала периода не может быть позже даты окончания")  # Бросаем ошибку значения
    if end_date > datetime.now():  # Проверяем, что конец периода не в будущем
        raise ValueError("Дата окончания периода не может быть в будущем")  # Бросаем ошибку значения

    days_count = (end_date - start_date).days + 1  # Считаем количество дней в периоде (включительно)
    if days_count <= 0:  # Проверяем, что период положительный
        raise ValueError("Некорректный период: количество дней должно быть положительным")  # Бросаем ошибку

    total_income = 1200000  # Временные данные суммарного дохода
    total_costs = 900000  # Временные данные суммарных расходов
    total_profit = total_income - total_costs  # Считаем общую прибыль
    avg_daily_income = total_income / days_count  # Среднесуточный доход
    avg_daily_profit = total_profit / days_count  # Среднесуточная прибыль

    return {  # Возвращаем итоговый отчёт за период
        'period': {'start': start_date.date(), 'end': end_date.date(), 'days': days_count},
        'totals': {'income': total_income, 'costs': total_costs, 'profit': total_profit},
        'averages': {'daily_income': round(avg_daily_income, 2), 'daily_profit': round(avg_daily_profit, 2)},
